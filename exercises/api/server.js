...
const findAsset = (name) => {
  ...
  return new Promise((resolve, reject) => { // Create a promise
    fs.readFile(assetPath, {encoding: 'utf-8'}, (error, result) => { // 3rd param of readFile takes a callback
      if (error){
        reject(error)
      } else {
        resolve(result)
...
const server = http.createServer( async (req, res) => { // Makes this function async
